...
float zero_disable_option_ratio = 0;
...
float learner_option_loss_scale = 1.0f;
...
bool nn_use_consistency = false;

// option parameters
int option_seq_length = 1;
...
std::string env_gridworld_maps_dir = "";
bool fix_player = false;
bool fix_goal = false;
...
void setConfiguration(ConfigureLoader& cl)
{
    ...
    cl.addParameter("zero_disable_option_ratio", zero_disable_option_ratio, "the probability to disable acting option in the environment", "Zero");
    ...
    cl.addParameter("learner_option_loss_scale", learner_option_loss_scale, "hyperparameter for scaling of the option loss", "Learner");
    ...
    cl.addParameter("nn_use_consistency", nn_use_consistency, "state consistency for MuZero network", "Network");

    // option parameters
    cl.addParameter("option_seq_length", option_seq_length, "the maximum option length", "Option");
...
#elif GRIDWORLD
    cl.addParameter("env_gridworld_maps_dir", env_gridworld_maps_dir, "board_initialization", "Enviroment");
    cl.addParameter("fix_player", fix_player, "fix the player to a predefined position", "Enviroment");
    cl.addParameter("fix_goal", fix_goal, "fix the goal to a predefined position", "Enviroment");
#endif
