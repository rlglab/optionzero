...
public:
    ...
    float* reward_;
    float* loss_scale_;
    float* option_loss_scale_;
    int* step_option_length_;
    int* step_unroll_length_;
    int* sampled_index_;
    std::vector<utils::Rotation> rotation_ = std::vector<utils::Rotation>(config::learner_batch_size, utils::Rotation::kRotationNone);
};
...
public:
    ...
    virtual void updateMax(int* sampled_index, int* batch_max_ids);
