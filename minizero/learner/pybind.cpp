...
PYBIND11_MODULE(minizero_py, m)
{
    ...
    m.def("get_option_loss_scale", []() { return config::learner_option_loss_scale; });
    ...
    m.def("get_nn_use_consistency", []() { return config::nn_use_consistency; });
    m.def("get_option_seq_length", []() { return config::option_seq_length; });
    m.def("get_option_action_size", []() { return getEnvInstance().getOptionPolicySize(); });
    ...
    py::class_<learner::DataLoader>(m, "DataLoader")
        ...
        .def(
            "update_max", [](learner::DataLoader& data_loader, py::array_t<int>& sampled_index, py::array_t<int>& batch_max_ids) {
                data_loader.updateMax(static_cast<int*>(sampled_index.request().ptr), static_cast<int*>(batch_max_ids.request().ptr));
            },
            py::call_guard<py::gil_scoped_release>())
        .def(
            "sample_data", [](learner::DataLoader& data_loader, py::array_t<float>& features, py::array_t<float>& action_features, py::array_t<float>& policy, py::array_t<float>& value, py::array_t<float>& option, py::array_t<float>& reward, py::array_t<float>& loss_scale, py::array_t<int>& step_option_length, py::array_t<int>& step_unroll_length, py::array_t<float>& option_loss_scale, py::array_t<int>& sampled_index) {
                ...
                data_loader.getSharedData()->getDataPtr()->option_ = static_cast<float*>(option.request().ptr);
                ...
                data_loader.getSharedData()->getDataPtr()->step_option_length_ = static_cast<int*>(step_option_length.request().ptr);
                data_loader.getSharedData()->getDataPtr()->step_unroll_length_ = static_cast<int*>(step_unroll_length.request().ptr);
                data_loader.getSharedData()->getDataPtr()->option_loss_scale_ = static_cast<float*>(option_loss_scale.request().ptr);
