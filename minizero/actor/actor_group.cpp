...
void ThreadSharedData::outputGame(const std::shared_ptr<BaseActor>& actor)
{
    ...
    if (!is_terminal) {
        ...
        for (int i = data_range.first; i <= data_range.second; ++i) {
            auto op1 = action_info_history[i].back();
            action_info_history[i].clear();
            action_info_history[i].shrink_to_fit();
            // for recovering the video, OP1 should be the last tag
            if (op1.first == "OP1") { action_info_history[i].push_back(op1); }
        }
    }
...
std::pair<int, int> ThreadSharedData::calculateTrainingDataRange(const std::shared_ptr<BaseActor>& actor)
{
    ...
    if (config::zero_actor_intermediate_sequence_length > 0) {
        data_end = std::max(0, (actor->getEnvironment().isTerminal() ? data_end : data_end - config::option_seq_length * config::learner_muzero_unrolling_step - config::learner_n_step_return));
...
void SlaveThread::handleSearchDone(int actor_id)
{
    ...
    if (is_endgame) {
        ...
        if (sequence_length > 0 && game_length >= sequence_length && (game_length - config::option_seq_length * config::learner_muzero_unrolling_step - config::learner_n_step_return) % sequence_length == 0) { getSharedData()->outputGame(actor); }
        actor->resetSearch();
    }
}
...
void ActorGroup::createActors()
{
    ...
    uint64_t tree_node_size = static_cast<uint64_t>(config::actor_num_simulation + 1) * (network->getActionSize() + 1) * config::option_seq_length;
