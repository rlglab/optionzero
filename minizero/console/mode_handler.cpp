...
ModeHandler::ModeHandler()
{
    ...
    RegisterFunction("decompress_str", this, &ModeHandler::runDecompressStr);
}
...
void ModeHandler::runZeroTrainingName()
{
    std::cout << Environment().name()                                                           // name for environment
              ...
              << "-" << GIT_SHORT_HASH                                                          // git hash info
              << "_"
              << "l" << config::option_seq_length                         // maximum option length
              << "_" << TimeSystem::getTimeString("YmdHis") << std::endl; // time stamp
}
...
void ModeHandler::runDecompressStr()
{
    std::string ss;
    while (std::cin >> ss) {
        std::cout << utils::decompressString(ss) << std::endl;
    }
}
