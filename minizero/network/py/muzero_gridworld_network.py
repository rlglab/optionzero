...
from .network_unit import ResidualBlock, PolicyNetwork, DiscreteValueNetwork, MuZeroConsistencyNetwork
...
class MuZeroPredictionNetwork(nn.Module):
    def __init__(self, num_channels, channel_height, channel_width, action_size, num_value_hidden_channels, value_size, option_seq_length, option_action_size):
        super(MuZeroPredictionNetwork, self).__init__()
        self.policy = PolicyNetwork(num_channels, channel_height, channel_width, action_size, option_seq_length, option_action_size)
        self.value = DiscreteValueNetwork(num_channels, channel_height, channel_width, num_value_hidden_channels, value_size)

    def forward(self, hidden_state):
        policy_logit, option_logit = self.policy(hidden_state)
        value_logit = self.value(hidden_state)
        return policy_logit, value_logit, option_logit


class MuZeroGridWorldNetwork(nn.Module):
    def __init__(self,
                 game_name,
                 num_input_channels,
                 ...
                 discrete_value_size):
        super(MuZeroGridWorldNetwork, self).__init__()
        ...
        self.prediction_network = MuZeroPredictionNetwork(num_hidden_channels, hidden_channel_height, hidden_channel_width, action_size,
                                                          num_value_hidden_channels, discrete_value_size, option_seq_length, option_action_size)
        self.consistency_network = MuZeroConsistencyNetwork(num_hidden_channels, hidden_channel_height, hidden_channel_width)

    @torch.jit.export
    def get_type_name(self):
        return "muzero_gridworld"
    ...
    @torch.jit.export
    def get_option_seq_length(self):
        return self.option_seq_length

    @torch.jit.export
    def get_option_action_size(self):
        return self.option_action_size
    ...
    def initial_inference(self, state):
        ...
        policy_logit, value_logit, option_logit = self.prediction_network(hidden_state)
        policy = torch.softmax(policy_logit, dim=1)
        value = torch.softmax(value_logit, dim=1)
        option = torch.softmax(option_logit, dim=2)
        return {"policy_logit": policy_logit,
                "policy": policy,
                "value_logit": value_logit,
                "value": value,
                "hidden_state": hidden_state,
                "option_logit": option_logit,
                "option": option}
    ...
    def recurrent_inference(self, hidden_state, action_plane):
        ...
        policy_logit, value_logit, option_logit = self.prediction_network(next_hidden_state)
        policy = torch.softmax(policy_logit, dim=1)
        value = torch.softmax(value_logit, dim=1)
    ...
    @torch.jit.export
    def get_consistency_proj(self, state, with_grad: bool):
        if not with_grad:
            state = self.representation_network(state)
            state = self.scale_hidden_state(state)
        return self.consistency_network(state, with_grad)
    ...
    def forward(self, state, action_plane=torch.empty(0), consistency: bool = False, with_grad: bool = False):
        if consistency:
            return self.get_consistency_proj(state, with_grad)
        else:
