...
const int kAtariFeatureHistorySize = 4;
...
public:
    ...
    {
        if (option_.empty()) { return getAtariActionName(action_id_); }
        std::string action_name = "";
        for (auto& action_id : option_) {
            action_name += (action_name == "" ? "" : "-") + getAtariActionName(action_id);
        }
        return action_name;
    }
};
...
public:
    ...
    bool isTerminal() const override { return (getEpisodeFrameNumber() >= kAtariMaxNumFramesPerEpisode) || ale_.game_over(false); }
    ...
    std::vector<float> getActionFeatures(const std::vector<AtariAction>& action, utils::Rotation rotation = utils::Rotation::kRotationNone) const;
...
public:
    ...
    std::vector<float> getPrimitiveReward(const int pos) const { return toDiscreteValue(pos < static_cast<int>(action_pairs_.size()) ? utils::transformValue(BaseEnvLoader::getReward(pos)[0]) : 0.0f); }
